...
interface CrosswordResultDialogProps {
  open: boolean;
...
export function CrosswordResultDialog({
  open,
  onOpenChange,
  isSuccess,
  onRetry,
    ...
    >
      <div className="flex flex-col h-full justify-center gap-3">
        ...
        <p className="text-base sm:text-lg text-slate-800 font-medium leading-snug">
          {isSuccess ? (
            <>
              Las tradiciones también{" "}
              <span className="font-bold text-slate-900">
                guardan historias.
              </span>
              <span className="font-bold text-slate-900 block">
                ¿Cuál de todas las celebraciones de tu época escolar esperabas
                con más emoción?
              </span>
            </>
          ) : (
